...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
// CPU implementation (optionally with OpenMP support)

template <typename scalar_t>
void calc_radial_psd_cpu(const at::Tensor& input,  // [B, H, W, C]
                         at::Tensor& psd) {
...
#if defined(WITH_CUDA)
  // Launch CUDA kernel
  launch_calc_radial_psd_kernel(input, psd);
#else
  // CPU implementation
  calc_radial_psd_cpu<double>(input, psd);
#endif
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  doc_stream << "  WITH_CUDA   : " << (is_compiled_with_cuda() ? "YES" : "NO");
  doc_stream << std::endl;
  doc_stream << "  WITH_OPENMP : " << (is_compiled_with_openmp() ? "YES" : "NO");
  doc_stream << std::endl;

  m.doc() = doc_stream.str().c_str();

  // Register the functions
  m.def("calc_radial_psd", &calc_radial_psd, "Calculate the radial profile of the power spectral density (PSD) of a 4D tensor");
  m.def("is_compiled_with_cuda", &is_compiled_with_cuda, "Check if the module is compiled with CUDA support");
  m.def("is_compiled_with_openmp", &is_compiled_with_openmp, "Check if the module is compiled with OpenMP support");
}
